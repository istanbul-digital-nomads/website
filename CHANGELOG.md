...
- **Ali Sameni's week now lives in the plans feed and on its own detail pages.** The seven showcase days (Kadıköy / Şişli / Beyoğlu) are mixed into the `/plans` feed as real plan cards (kept visible regardless of the date range, still honouring the neighborhood/vibe filters), and each day has its own public detail page at `/plans/ali-week/<day>` - hero, route map, stop-by-stop timeline, and prev/next day nav - mirroring the regular plan-detail layout. The `/map` link and the viewer copy now frame it as Ali's real week, not a sample. Fixed the animated viewer failing to load on client-side navigation (it now flags the map ready and resizes the canvas on warm-cache remounts, so the camera engages instead of sitting blank).

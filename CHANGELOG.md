...
- **CI formatting check.** Prettier pass on `src/lib/analytics.ts` (the EEA region list was hand-wrapped) - formatting only, no behavior change.
...
- **Share modal is now vertically centered on mobile.** It used to stick to the bottom of the screen as a sheet; now it's centered both ways on every viewport, rounds all four corners, scrolls if it's taller than the screen, and fades + scales in for a cleaner open.
...
- **"Share to Stories" attaches the image again.** The `instagram://story-camera` deeplink from v3.30.2 opened Instagram but with no image - the web camera deeplink can't carry one. Instagram's image-attached share (`instagram-stories://share` + iOS `UIPasteboard` keys / Android `ADD_TO_STORY` intent, gated on a Facebook App ID) is native-app-only - it's what the X app uses and isn't reachable from a website. Reverted to the Web Share API with the image file, the only web mechanism that hands the image to Instagram: choose Instagram in the share sheet and the story composer opens with the image loaded (download fallback on desktop).

  ...
  return (
    <div style={{
      ...
      <div style={{display:'flex', alignItems:'center', gap:14}}>
        <div style={{
          width:28, height:28, border:'1px solid var(--paper)', borderRadius:'50%',
          position:'relative', display:'grid', placeItems:'center',
        }}>
          <div style={{width:6, height:6, background:'var(--terracotta)', borderRadius:'50%'}}/>
          <div style={{position:'absolute', inset:0, borderTop:'1px solid var(--paper)', transform:'rotate(45deg)', borderRadius:'50%'}}/>
        </div>
        <div style={{display:'flex', flexDirection:'column', gap:2}}>
          <div style={{fontFamily:'var(--serif)', fontSize:17, letterSpacing:'-0.01em', lineHeight:1}}>Istanbul Nomads</div>
          <div className="in-mono" style={{fontSize:9.5, color:'var(--paper-mute)'}}>Made in Kadıköy · Est. 2023</div>
        </div>
      </div>
      ...
      <div style={{display:'flex', alignItems:'center', gap:18}}>
        <button style={{
          padding:'7px 12px', border:'1px solid var(--ink-4)', borderRadius:4,
          fontSize:11.5, color:'var(--paper-mute)', fontFamily:'var(--mono)',
          display:'flex', alignItems:'center', gap:8,
        }}>
          <span style={{color:'var(--paper-faint)'}}>Search</span>
          <span style={{padding:'1px 6px', border:'1px solid var(--ink-4)', borderRadius:2, fontSize:10, color:'var(--paper-mute)'}}>⌘K</span>
        </button>
        <div className="in-mono" style={{fontSize:10.5, color:'var(--paper-mute)', display:'flex', gap:10}}>
          <span style={{color:'var(--paper)'}}>{lang}</span>
          <span>TR</span><span>FA</span><span>AR</span><span>RU</span>
        </div>
        <button style={{
          padding:'8px 14px', border:'1px solid var(--ink-4)', borderRadius:999,
          fontSize:12.5, color:'var(--paper)',
          display:'flex', alignItems:'center', gap:8,
        }}>
          <span style={{width:6, height:6, background:'#26a69a', borderRadius:'50%', boxShadow:'0 0 8px #26a69a'}}/>
          Telegram
        </button>
        <button style={{padding:'8px 14px', background:'var(--terracotta)', color:'var(--ink-0)', borderRadius:2, fontSize:12.5, fontWeight:500}}>
          Sign in
        </button>
      </div>
    </div>
...
const Footer = () => (
  <div style={{
    padding:'80px 56px 48px', borderTop:'1px solid var(--ink-3)',
    background:'var(--ink-1)',
  }}>
    <div style={{display:'grid', gridTemplateColumns:'2.2fr 1fr 1fr 1fr 1fr', gap:48, marginBottom:80}}>
      <div>
        <div style={{fontFamily:'var(--serif)', fontSize:36, lineHeight:1.05, letterSpacing:'-0.02em', maxWidth:380}}>
          Built on the Asia side. Read on every side.
        </div>
        <div className="in-mono" style={{fontSize:11, color:'var(--paper-mute)', marginTop:24, lineHeight:1.7}}>
          ISTANBUL · 41°00′N 28°58′E<br/>
          UTC+3 · TRY ₺ · EN/TR/FA/AR/RU
        </div>
      </div>
      {[
        ...
        <div key={h}>
          <div className="in-mono" style={{fontSize:10.5, color:'var(--paper-mute)', marginBottom:18}}>{h}</div>
    ...
    <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-end', paddingTop:32, borderTop:'1px solid var(--ink-3)'}}>

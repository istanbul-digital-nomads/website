    ...
    <div className="space-y-2">
      {(Object.keys(SCORE_LABELS) as (keyof NomadScores)[]).map((key) => (
        <div key={key} className="flex items-center gap-2">
          <span className="w-14 shrink-0 text-xs text-neutral-500 dark:text-[#8a7a6a]">
            {SCORE_LABELS[key]}
          </span>
          ...
          <span className="w-4 text-right text-xs font-medium text-neutral-700 dark:text-[#d4c4b4]">
            {scores[key]}
          </span>
        </div>
      ))}

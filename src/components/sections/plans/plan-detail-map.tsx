...
export function PlanDetailMap({ stops }: Props) {
  ...
  // Fit the map to all pins once the style has loaded.
  // Runs on both mapLoaded and positioned changes.
  ...
  const handleLoad = useCallback(() => {
    setMapLoaded(true);
    // fitBounds right after load since the useCallback ref will be stale
    // until the next render - call directly.
    const map = mapRef.current?.getMap();
    if (!map || positioned.length === 0) return;
    if (positioned.length === 1) {
      map.flyTo({ center: [positioned[0]!.pos.lng, positioned[0]!.pos.lat], zoom: 14, duration: 600 });
